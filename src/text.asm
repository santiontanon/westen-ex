...
; - bc: text offset
; OLD - c: bank #
; OLD - a: text # within the bank
; output:
; - de: pointer to copy the decompressed text to (ignored, hardcoded to text_buffer)
get_text_from_bank_reusing_bank_multilingual:
	ld hl,(current_language_table)
	add hl,bc
	ld c,(hl)
	inc hl
	ld a,(hl)
...
get_text_from_bank_multilingual:
	ld hl,(current_language_table)
	add hl,bc
	SETMEGAROMPAGE_A000 TEXT_MULTILINGUAL_TABLE_PAGE	
	ld c,(hl)  ; bank
	inc hl
	ld a,(hl)  ; index
...
get_text_from_bank_entry_point1:	
	...
	push af
		...
		ld hl,(current_language_bank_ptr)
		ld b,0
		add hl,bc
		add hl,bc
		ld e,(hl)
...
draw_text_from_bank_reusing_multilingual:
	push bc
    push de
    push iy
;         ld de,text_buffer
        call get_text_from_bank_reusing_bank_multilingual
...
; ------------------------------------------------
; input:
; - a: expected length in bytes
; - bc: text offset
; - de: address to render
; - iyl: color
; - iyh: pixel to start from (128 for starting on pixel 0)
draw_text_from_bank_multilingual:
	ld iyh,128  ; start in pixel 0
draw_text_from_bank_multilingual_iyh_set:
	push af
	push bc
    push de
    push iy
        call get_text_from_bank_multilingual
