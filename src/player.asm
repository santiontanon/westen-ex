...
; 	jp nz,inventory_fn_jump
	rra  ; since KEY_BUTTON1_BIT == 0
	jp c,inventory_fn_jump
...
get_map_ptr_from_room_number:
	...
	cp #40
	jr z,enter_door_map5
; 	cp #50
; 	jr z,enter_door_map6
enter_door_map6:
	ld hl,map6_zx0
	ret
enter_door_map5:
...
get_nearest_door:
	...
	ld a,(n_doors)
	or a
	jr z,get_nearest_door_not_found
	ld b,a
...
draw_player_internal:
	...
	push hl
		ex de,hl
		ld hl,player_sprite_occlusion_buffer+1
		call copy_masked_sprite_to_vdp_setwrt_set
	pop hl
	ld bc,32
	add hl,bc
	push hl
		ex de,hl
		ld hl,player_sprite_occlusion_buffer+2
		call copy_masked_sprite_to_vdp_setwrt_set
	pop hl
	ld bc,32
	add hl,bc
	push hl
		ex de,hl
		ld hl,player_sprite_occlusion_buffer+17
		call copy_masked_sprite_to_vdp_setwrt_set
	pop hl
	ld bc,32
	add hl,bc
...
calculate_player_occlusion_mask_loop:
	push bc
		...
		push af
			call c,add_object_to_player_occlusion_mask
		pop af
		call nc,add_enemy_to_player_occlusion_mask
